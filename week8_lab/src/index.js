...
function CardTemplate(parentEl, headerText, bodyText, imgUrl) {
  ...
  myButtonVariable.onclick = function() {
    const dog = {header: headerText, body: bodyText, img: imgUrl};
...
function onLoad() {
  ...
  if (myDog === null){
    ...
    const savedDog = JSON.parse(myDog);
    CardTemplate(divDoggoContainer, savedDog.header, savedDog.body, savedDog.img);
  }
}
